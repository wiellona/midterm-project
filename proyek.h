...
void menu_searching(int jumlah_database, struct Event *event1) {
	FILE *data;
	data = fopen("data proyek.txt", "w");
	struct Event *event1;
	int database, pilihan;
	...
	do {
		...
		{
		case 1:
			searching_nama_event(&event1[database], &jumlah_database, data);
			break;
		case 2:
			searching_waktu_event(&event1[database], &jumlah_database, data);
			break;
		case 3:
			searching_tipe_event(&event1[database], &jumlah_database, data);
			break;
		case 4:
			searching_urgensi_event(&event1[database], &jumlah_database, data);
			break;
		case 5:
			return;
		default:
			printf("Input tidak valid. Silahkan masukkan angka 1-5\n");
			break;
		}
	} while (pilihan != 5);
}
